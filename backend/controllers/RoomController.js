...
const updateRoom = async (req, res) => {
  ...
  try {
    ...
    const [userCheck] = await db.promise().query("SELECT * FROM room WHERE room_Number = ?", [roomNumber]);
    ...
    const updateQuery = `
      ...
      WHERE room_Number = ?
    `;
    await db.promise().query(updateQuery, [roomnumber, room_stat_ID, roomNumber]);
...
const updateRoomStatus = async (req, res) => {
  ...
  try {
    if (!roomNumber || !room_stat_ID) {
      return res.status(400).json({ error: "กรุณาระบุ roomNumber และ room_stat_ID" });
    }
    const [userCheck] = await db.promise().query("SELECT * FROM room WHERE room_Number = ?", [roomNumber]);
    if (userCheck.length === 0) {
      return res.status(404).json({ error: "ไม่พบข้อมูลห้องพัก" });
    ...
    const updateQuery = "UPDATE room SET room_stat_ID = ? WHERE room_Number = ?";
    await db.promise().query(updateQuery, [room_stat_ID, roomNumber]);
...
const getStatusRoom = async (req, res) => {
  try {
    const query = 'SELECT stat_ID,stat_Name FROM status WHERE stat_StatTypID = "STT000002"';
    const [result] = await db.promise().query(query);
    res.status(200).json(result);
  } catch (err) {
    console.error("เกิดข้อผิดพลาด:", err);
